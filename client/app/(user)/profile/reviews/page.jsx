...
import { Star, MessageSquare, ChevronRight, PackageSearch } from 'lucide-react';
        ...
        <div className="relative px-5 md:px-8 py-6 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-xs text-[#94a3b8] uppercase tracking-widest font-semibold">{reviews?.data?.length} reviews written</p>
            </div>
          </div>
          ...
          >
            ...
            <div className="px-4 md:px-6 py-4 border-b border-[#f1f5f9] bg-[#f8faff] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="w-20 h-20 shrink-0 rounded-xl bg-white border border-[#e8edf5] overflow-hidden p-1.5 group-hover:border-accent/25 group-hover:shadow-[0_4px_12px_#155dfc14] transition-all duration-300">
                  <img
                    src={review.product.variants[0].thumbnail}
                ...
                <div className="min-w-0">
                  ...
                  <h3 className="text-sm font-bold text-[#0f172a] mt-1 truncate">{review.product.title}</h3>
                </div>
              </div>

              {/* View details button */}
              <Link href={`/product/${review.product.slug}`} className="shrink-0 inline-flex items-center gap-1.5 text-[11px] font-bold text-accent border border-accent/20 bg-[#eff6ff] px-3.5 py-1.5 rounded-full hover:bg-accent hover:text-white hover:border-accent hover:shadow-[0_4px_10px_#155dfc33] transition-all duration-200 cursor-pointer uppercase tracking-wide">
            ...
            <div className="px-4 md:px-6 py-4 md:py-5">
              ...
              <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                ...
                <span className="text-[12px] text-[#7c8796] font-mono">{new Date(review?.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }).toLowerCase()}</span>
              </div>
