...
const checkoutSingle = async (req, res) => {
    try {
        ...
        const { paymentMethod, slug, sku, quantity, fullname, shippingAddress, city, division, phone } = req.body

        // ------- Validation
        if (!fullname) return resHandler.error(res, 400, "Fullname is required")
        if (!paymentMethod) return resHandler.error(res, 400, "Payment type is required")
        if (!slug) return resHandler.error(res, 400, "Unauthorized request")
...
module.exports = { checkout, checkoutSingle, webhook, getOrders, getUserOrders }