    ...
    return (
        <div className="rounded-2xl border border-border overflow-hidden group relative bg-bg hover:shadow-xl transition-all duration-300">
            <div className="relative h-60 flex items-center justify-center bg-surface overflow-hidden">
                <img src={product.variants[0].thumbnail} alt={product.title} className="h-44 object-contain transition-transform duration-500 group-hover:scale-107" />
            ...
            <div className="px-4 py-4">
                ...
                <h3 className="text-[15px] font-semibold text-text-primary leading-snug line-clamp-1">{product.title}</h3>
                <div className="mt-3 flex items-center justify-between">
                    <span className="text-[17px] font-bold text-text-primary">${product.variants[0].price}</span>
                    <div className="flex items-center gap-1 text-xs text-text-secondary">
                        <Star size={12} className="text-yellow-400 fill-yellow-400" />
                        <span className="font-medium text-text-primary">{product.avgRating || 0}</span>
                        <span>({product.totalRating || 0})</span>
                    </div>
                </div>
            </div>
